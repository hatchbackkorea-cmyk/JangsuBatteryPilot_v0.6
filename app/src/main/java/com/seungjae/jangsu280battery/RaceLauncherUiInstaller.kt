...
object RaceLauncherUiInstaller {
    ...
    private const val TAG_CAMERA_BUTTON = "camera_gate_beta_launcher_button_v2"
    ...
    fun install(activity: Activity) {
        ...
        if (parent.findViewWithTag<View>(TAG_CAMERA_BUTTON) == null) {
            val cameraButton = Button(activity).apply {
                ...
                text = "📷 CAMERA GATE BETA v2\nSTART 트리거 · 안정 FPS · 정밀 시간 동기화"
                textSize = 17f
                setTypeface(typeface, Typeface.BOLD)
                isAllCaps = false
                layoutParams = LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(78)).apply { topMargin = dp(8) }
                setOnClickListener { activity.startActivity(Intent(activity, CameraGateTestActivityV2::class.java)) }
            }
