...
class RaceDataStore(context: Context) {
    ...
    data class Snapshot(
        ...
        val lastGateAtMs: Long = 0L,
        val elapsedMs: Long = 0L,
        val routeM: Double = 0.0,
        val totalM: Double = 0.0,
        val deltaMs: Long? = null,
        val nextGateIndex: Int = 0,
        val currentSector: String = "",
        val gpsAccuracyM: Double = 0.0,
        val maxSpeedKph: Double = 0.0,
        val maxGpsAccuracyM: Double = 0.0,
        val maxOffRouteM: Double = 0.0,
        val jumpCount: Int = 0,
        val validation: String = "REVIEW",
        val sectors: List<RaceSectorResult> = emptyList(),
        ...
        companion object {
            fun fromJson(o: JSONObject): Snapshot {
                ...
                return Snapshot(
                    ...
                    if (o.has("finish_rank") && !o.isNull("finish_rank")) o.optInt("finish_rank") else null,
                    o.optString("server_status", "")
                )
            }
        }
    }
    ...
    fun activeConfig(): Pair<RaceEventConfig, String>? = runCatching {
        val o = JSONObject(prefs.getString("active_config", ""))
        RaceEventConfig.fromJson(o) to o.optString("local_course_id")
    }.getOrNull()
    ...
    @Synchronized fun saveCompleted(summary: RaceRunSummary) {
        val current = runCatching { JSONArray(completedFile.readText(Charsets.UTF_8)) }.getOrDefault(JSONArray())
        val next = JSONArray(); var replaced = false
        for (i in 0 until current.length()) {
            val old = current.optJSONObject(i) ?: continue
            if (old.optString("run_id") == summary.runId) { next.put(summary.toJson()); replaced = true } else next.put(old)
        }
        if (!replaced) next.put(summary.toJson())
        val tmp = File(dir, "completed_runs.tmp")
        tmp.writeText(next.toString(), Charsets.UTF_8); tmp.copyTo(completedFile, overwrite = true); tmp.delete()
    }
    ...
    fun completed(): List<RaceRunSummary> {
        ...
        return (0 until a.length()).mapNotNull { a.optJSONObject(it)?.let { o -> runCatching { RaceRunSummary.fromJson(o) }.getOrNull() } }
    }
    fun nextRunNumber(eventCode: String): Int = completed().count { it.eventCode == eventCode } + 1

    fun saveJoined(config: RaceEventConfig, token: String, localCourseId: String) {
        prefs.edit().putString("joined_${config.eventCode}", JSONObject().apply {
            put("config", config.toJson()); put("token", token); put("local_course_id", localCourseId)
        }.toString()).apply()
        prefs.edit().putString("last_event_code", config.eventCode).apply()
    }
    data class Joined(val config: RaceEventConfig, val token: String, val localCourseId: String)
    fun joined(eventCode: String): Joined? = runCatching {
        val o = JSONObject(prefs.getString("joined_${eventCode.uppercase()}", ""))
        Joined(RaceEventConfig.fromJson(o.getJSONObject("config")), o.optString("token"), o.optString("local_course_id"))
    }.getOrNull()
    fun lastJoined(): Joined? = prefs.getString("last_event_code", null)?.let(::joined)

    @Synchronized fun enqueue(type: String, eventCode: String, payload: JSONObject) {
        val a = runCatching { JSONArray(prefs.getString("network_queue", "[]")) }.getOrDefault(JSONArray())
        val key = "$type:${payload.optString("run_id")}:${payload.optInt("sector_index", -1)}"
        val out = JSONArray(); var replaced = false
        for (i in 0 until a.length()) {
            val o = a.optJSONObject(i) ?: continue
            if (o.optString("key") == key) { out.put(JSONObject().apply { put("key", key); put("type", type); put("event_code", eventCode); put("payload", payload) }); replaced = true }
            else out.put(o)
        }
        if (!replaced) out.put(JSONObject().apply { put("key", key); put("type", type); put("event_code", eventCode); put("payload", payload) })
        prefs.edit().putString("network_queue", out.toString()).apply()
    }

    @Synchronized fun queued(): List<JSONObject> {
        val a = runCatching { JSONArray(prefs.getString("network_queue", "[]")) }.getOrDefault(JSONArray())
        return (0 until a.length()).mapNotNull(a::optJSONObject)
    }
    @Synchronized fun removeQueued(key: String) {
        val a = runCatching { JSONArray(prefs.getString("network_queue", "[]")) }.getOrDefault(JSONArray()); val out = JSONArray()
        for (i in 0 until a.length()) a.optJSONObject(i)?.takeIf { it.optString("key") != key }?.let(out::put)
        prefs.edit().putString("network_queue", out.toString()).apply()
    }
}
