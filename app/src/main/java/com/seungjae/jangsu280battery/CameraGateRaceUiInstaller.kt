 ...
 *
 * RaceActivity builds its UI programmatically and may replace the content view while the same
 * Activity stays resumed. Therefore this installer watches layout changes and re-attaches the
 * button whenever the RACE home (identified by its large START button) is rebuilt.
 */
object CameraGateRaceUiInstaller {
    private const val TAG_CAMERA_BUTTON = "camera_gate_beta_race_home_v1"
    ...
    private fun attachIfHome(activity: Activity) {
        ...
        // HOME body contains: event status, spacer, START, hint, spacer, menuRow, footer.
        // Put Camera Gate directly under the player/event menu row so it is always visible.
