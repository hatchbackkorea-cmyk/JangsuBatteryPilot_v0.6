...
class TimeGateLabActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!sync.isAdminDeviceCached()) {
            Toast.makeText(this, "실험실은 관리자 핸드폰에서만 열 수 있습니다.", Toast.LENGTH_LONG).show()
            finish()
            return
        }
        ...
        if (sync.configured()) {
            sync.checkAdminStatusAsync { result ->
                if (!result.ok && !sync.isAdminDeviceCached()) runOnUiThread {
                    Toast.makeText(this, "관리자폰 권한이 확인되지 않아 실험실을 닫습니다.", Toast.LENGTH_LONG).show()
                    finish()
                }
            }
        }
    ...
    private fun buildUi() {
        ...
        body.addView(header("실험실"))

        val test = panelBox()
        test.addView(title("테스트 모드"))
        test.addView(note(runCatching {
            val m = repo.activeMeta()
            "테스트 기준 코스 · ${m.name} · ${RideFormatter.one(m.totalKm)} km"
        }.getOrDefault("선택 코스 정보를 불러오지 못했습니다.")))

        switchTest = Switch(this).apply {
            text = "GPS 대신 테스트 위치 사용"
        ...
        tvKm = label("")
        test.addView(tvKm)
        seekKm = SeekBar(this).apply {
            max = (totalKm * 10.0).roundToInt().coerceAtLeast(1)
            progress = (AppSettings.testKm(this@TimeGateLabActivity).coerceIn(0.0, totalKm) * 10.0).roundToInt()
        }
        test.addView(seekKm, LinearLayout.LayoutParams(-1, dp(48)))
        tvHint = note("")
        test.addView(tvHint)
        ...
        test.addView(Button(this).apply {
            text = "진행 위치 / 실제 배터리 보정 초기화"
            isAllCaps = false
            setOnClickListener { resetProgress() }
        }, LinearLayout.LayoutParams(-1, dp(48)).apply { topMargin = dp(8) })
        body.addView(test, panelLp())

        val deploy = panelBox(accent = true)
        deploy.addView(title("📦 모바일 소스 배포"))
        deploy.addView(note("전체 소스 ZIP을 선택해 GitHub main에 배포하고 signed Release APK 생성까지 이어서 확인합니다."))
        deploy.addView(Button(this).apply {
            text = "📦 새 ZIP 모바일 배포 열기"
            isAllCaps = false
            setTypeface(typeface, Typeface.BOLD)
            setOnClickListener { startActivity(Intent(this@TimeGateLabActivity, ReleaseUploaderActivity::class.java)) }
    ...
    private fun resetProgress() {
        ...
        AlertDialog.Builder(this)
            .setTitle("진행 기록 초기화")
            .setMessage("현재 코스 진행 위치와 실제 배터리 보정값을 0km 상태로 초기화할까요?")
    ...
    private fun header(name: String) = LinearLayout(this).apply {
        ...
        addView(Button(this@TimeGateLabActivity).apply {
            text = "‹"
            textSize = 26f
            setOnClickListener { finish() }
        }, LinearLayout.LayoutParams(dp(58), dp(52)))
        addView(TextView(this@TimeGateLabActivity).apply {
            text = name
            textSize = 26f
            setTextColor(primary)
            setTypeface(typeface, Typeface.BOLD)
