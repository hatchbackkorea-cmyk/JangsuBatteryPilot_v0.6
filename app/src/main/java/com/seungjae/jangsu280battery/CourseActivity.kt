...
class CourseActivity : Activity() {
    ...
    private fun renderChargingPlan(meta: CourseMeta, riding: Boolean) {
        ...
        val planner = EnergyTripPlanner(planForStations)
        stations.forEachIndexed { index, station ->
            val advice = planner.adviceAtStation(station.routeKm, AppSettings.finishTarget(this))
            chargingContainer.addView(buildStationRow(meta, station, index + 1, riding, advice))
        }
    }

    private fun buildStationRow(meta: CourseMeta, station: ChargingStation, number: Int, riding: Boolean, advice: ChargePlanAdvice?): View {
    ...
    private fun addRecommendedStation(meta: CourseMeta) {
        ...
        val km = BatteryPlan(course, learningStore, emptyList()).recommendedChargeKm(AppSettings.finishTarget(this))
        if (km == null) {
            AlertDialog.Builder(this)
                .setTitle("자동 충전 분석")
            ...
            REQ_GPX -> {
                ...
                try {
                    ...
                    if (wptCount > 0) {
                        ...
                        val recommended = BatteryPlan(course, learningStore, emptyList()).recommendedChargeKm(AppSettings.finishTarget(this))
                        AlertDialog.Builder(this)
