...
val updateRepository = (project.findProperty("updateRepo")?.toString()
    ...
    ?: "").trim()
val defaultRaceServerUrl = runCatching {
    val text = rootProject.file("rcc-server.json").readText()
    Regex("\\\"url\\\"\\s*:\\s*\\\"([^\\\"]+)\\\"").find(text)?.groupValues?.getOrNull(1).orEmpty()
}.getOrDefault("").trim().trimEnd('/')
...
android {
    ...
    defaultConfig {
        ...
        val escapedRaceServer = defaultRaceServerUrl.replace("\\", "\\\\").replace("\"", "\\\"")
        buildConfigField("String", "DEFAULT_RACE_SERVER_URL", "\"$escapedRaceServer\"")
